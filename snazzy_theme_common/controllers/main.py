...
class BizopleWebsiteSale(WebsiteSale):
    ...
    def _get_search_options(
        ...
        res = super(BizopleWebsiteSale, self)._get_search_options(category, attrib_values, tags, min_price, max_price, conversion_rate, **post)
...
class PwaMain(http.Controller):
    ...
    def search_autocomplete(self, term=None, category=None):
        ...
        #     if prod_category.child_id and prod_category.child_id.product_tmpl_ids:
        #         for product in prod_category.child_id.product_tmpl_ids:
        #             product_list.append(product.id)
        #     results = request.env["product.template"].sudo().search([('name','ilike',term),('id','in',product_list)])
        #     value ={
        #         'results' :results
        #     }
        #     return request.render("theme_snazzy.search_snazzy",value)
        # else:
