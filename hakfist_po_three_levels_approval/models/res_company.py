...
class ResCompany(models.Model):
    ...
    def _set_template_and_amt(self):
        if self.three_step_approval:
            self.update({
                'approve_mail_template': self.env.ref('hakfist_po_three_levels_approval.purchase_order_approval_email_template').id,
                'refuse_mail_template': self.env.ref('hakfist_po_three_levels_approval.purchase_order_refuse_email_template').id
            })
        else:
