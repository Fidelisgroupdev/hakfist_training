...
class SaleOrder(models.Model):
    ...
    packing_id = fields.Many2one('packaging.method', string="Packaging Method")
    ...
    def action_open_packing_wizard(self):
        view_id = self.env.ref('hak_packgaing_method.choose_packaging_method_view_form').id
        ...
        #     )
        return {
            'name': _('Add a packaging method'),
            'type': 'ir.actions.act_window',
            'view_mode': 'form',
            'res_model': 'choose.packaging.method',
            'view_id': view_id,
            'views': [(view_id, 'form')],
            'target': 'new',
            'context': {
    ...
    def set_package_line(self, package, amount):
        # self._remove_delivery_line()
        for order in self:
            order.packing_id = package.id
            order._create_package_line(package, amount)
        return True


    def _create_package_line(self, package, price_unit):
        values = self._prepare_package_line_vals(package, price_unit)
        return self.env['sale.order.line'].sudo().create(values)


    def _prepare_package_line_vals(self, package, price_unit):
        context = {}
        if self.partner_id:
            # set delivery detail in the customer language
            context['lang'] = self.partner_id.lang
            package = package.with_context(lang=self.partner_id.lang)

        # Apply fiscal position
        taxes = package.product_id.taxes_id.filtered(lambda t: t.company_id.id == self.company_id.id)
        taxes_ids = taxes.ids
        ...
        if package.product_id.description_sale:
            so_description = '%s: %s' % (package.name,
                                        package.product_id.description_sale)
        else:
            so_description = package.name
        values = {
            'order_id': self.id,
            'name': so_description,
